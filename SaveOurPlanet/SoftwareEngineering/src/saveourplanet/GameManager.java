...
public class GameManager {
	...
	private static void startGame() {
		...
		while (true) {
			do {
				try {
					System.out.println("If you are you ready to begin the game, enter 1 and then press Enter");
					System.out.println("If you would like more instructions, enter 2 and then press Enter");
					System.out.println("If you would like to quit, enter 3 and then press Enter");
			...
			if (input == 3) { // clearing newline for getting input in the quit after this
//				scanner.nextLine();

			}

			// if 3, or if they chose to quit from 1
			if (confirmQuit()) {
				scanner.close();
				return;
			}
	...
	private static boolean playTheGame() {
		...
		System.out.println("To quit, enter any other key and then press Enter");
	...
	private static int getPlayerCount() {
		...
		int playerCount = 0;
		do {
			System.out.println("Enter the number of players for this game (Minimium 2, Maximium 4) and then press Enter");
			try {
	...
	private static int getGameLength() {
		...
		do {
			sizeRangeCheck = true;
			System.out.println("Enter the number of turns for this game, enter 10, 20 or 30 and then press Enter");
			try {
	...
	public static void takeTurn() {
		...
		do {
			for (Player player : players) {
				...
				System.out.println("\n" + player.getPlayerName() + ", press Enter to roll the dice");
				scanner.nextLine();//
...
//						}
				...
				// that also)
				System.out.println("Quit Game?/Continue Game? (y/any other key) and then press Enter");// temp message - change later
				String answer = scanner.nextLine();//
				if (answer.equalsIgnoreCase("y")) {
	...
	public static void propertyValidation(Player player) {
		...
		if (property.getPropertyOwner() == null) {
			...
			System.out.println(property.getSquareName() + " is available for "+ property.getPurchaseCost()+
					". Would you like to buy? (Press Y/N )");
